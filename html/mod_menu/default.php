...
?>
<ul<?php echo $id; ?> class="navbar-nav mod-menu mod-list nav <?php echo $class_sfx; ?>">
    ...
    if ($item->deeper) {
        ...
        if ((int) $item->level === $startLevel) {
            switch ($item->type) {
                ...
                default:
                    // Il toggle è gestito da data-bs-toggle="dropdown" sul nav-link
                    break;
            }
        }
        $subClass = ((int) $item->level === $startLevel) ? 'dropdown-menu' : 'mod-menu__sub list-unstyled small';
